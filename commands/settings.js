...
module.exports = {
	...
	.addChannelOption(option => option.setName('counting').setDescription('Global Counting channel (Requires Admin)'))
    .toJSON(),
    ...
    async execute(interaction, data, client) {
        ...
        if(interaction.options.getChannel(`counting`)){
            ...
            arr.push(interaction.options.getChannel(`counting`).id.toString())
